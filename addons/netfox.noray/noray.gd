...
func _handle_commands(command: String, data: String):
	if command == "set-oid":
		...
		if parts.size() >= 2:
			...
			# Validar que host e porta são válidos
			if host.is_empty() or port_str.is_empty():
				_logger.error("Invalid connect command: empty host or port in %s", [data])
				return
			var port = port_str.to_int()
			if port <= 0 or port > 65535:
				_logger.error("Invalid connect command: invalid port %s in %s", [port_str, data])
				return
			_logger.debug("Received connect command to %s:%s", [host, port])
			on_connect_nat.emit(host, port)
		else:
			_logger.error("Invalid connect command format: %s", [data])
	elif command == "connect-relay":
		var host = _address
		var port_str = data.strip_edges()
		if port_str.is_empty():
			_logger.error("Invalid connect-relay command: empty port")
			return
		var port = port_str.to_int()
		if port <= 0 or port > 65535:
			_logger.error("Invalid connect-relay command: invalid port %s", [port_str])
			return
